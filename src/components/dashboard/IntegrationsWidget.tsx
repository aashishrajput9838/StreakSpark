...
const IntegrationsWidget: React.FC = () => {
  ...
  return (
    <div className="bg-appPalette-dark-card rounded-xl p-6 shadow flex flex-col gap-4 items-center">
      <div className="flex items-center gap-2 w-full mb-4">
        <img src="https://img.icons8.com/color/48/spotify--v1.png" alt="Spotify" className="w-10 h-10" />
        <div className="flex-1">
          <div className="font-bold text-lg text-appPalette-dark-text">Spotify Integration</div>
          <div className="text-sm text-appPalette-dark-muted">Empower yourself with habit tracking while enjoying uninterrupted music</div>
        </div>
      </div>
      ...
      {!isSpotifyConnected ? (
        <div className="w-full flex flex-col items-center gap-3">
          <button
            ...
            disabled={loading}
            className="bg-appPalette-orange text-white rounded-full px-6 py-3 w-full font-bold text-lg hover:bg-appPalette-pink focus:outline-none focus:ring-4 focus:ring-appPalette-pink transition-all duration-200 ease-in-out shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? 'Connecting...' : 'Connect Spotify'}
          </button>
          <button className="bg-appPalette-dark-border text-appPalette-dark-text rounded-lg px-4 py-2 w-full font-semibold text-sm hover:bg-appPalette-dark-card transition-colors duration-200 ease-in-out">
            More Integrations
        ...
        <div className="w-full flex flex-col gap-4">
          <p className="text-center text-appPalette-blue font-semibold">Spotify Connected! Enjoy your music.</p>
          {currentlyPlaying ? (
            <div className="bg-appPalette-dark-background rounded-lg p-4 flex items-center gap-4 shadow-inner">
              <img src={currentlyPlaying.albumArt} alt="Album Art" className="w-16 h-16 rounded-md shadow" />
              <div className="flex-1">
                <p className="font-bold text-appPalette-dark-text text-base leading-tight">{currentlyPlaying.name}</p>
                <p className="text-sm text-appPalette-dark-muted">{currentlyPlaying.artist}</p>
              </div>
              <div className="flex gap-2 items-center">
                <button
                  onClick={() => controlPlayback('previous')}
                  className="text-appPalette-dark-muted hover:text-appPalette-dark-text transition-colors duration-150"
                >
                ...
                <button
                  ...
                  className="text-appPalette-dark-text hover:text-white transition-colors duration-150 text-2xl"
                >
            ...
            <p className="text-center text-appPalette-dark-muted">No music currently playing.</p>
          )}
          <button
            onClick={() => setIsSpotifyConnected(false)} // Simulate disconnect
            className="mt-2 bg-appPalette-pink text-white rounded-lg px-4 py-2 w-full font-semibold hover:bg-appPalette-orange transition-colors duration-200 ease-in-out shadow-sm"
          >
            Disconnect Spotify
          </button>
        </div>
